class ApplicationController < ActionController::Base
	...
	def store_location
		...
		session[:previous_url] = request.fullpath unless request.fullpath =~ session[:previous_url] || request.fullpath =~ /\/users/ || request.fullpath =~ /\/ads\/new/ || request.fullpath =~ /\/messages\/new/ || request.fullpath =~ /\/contact-info/ || request.fullpath =~ /\/terms/ || request.fullpath.include?(".json")
	end
